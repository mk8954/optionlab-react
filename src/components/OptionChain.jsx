...
export default function OptionChain({ onBack, initialMarket = 'NIFTY', marketParameters, onSelectOption }) {
  const [market, setMarket] = useState(initialMarket);

  // Base daily change metrics
  const defaultParams = {
    NIFTY: { price: 24358.15, change: 252.80, pct: 1.05, expiry: '2026-08-04' },
    BANKNIFTY: { price: 52945.00, change: -112.80, pct: -0.21, expiry: '2026-08-25' },
    FINNIFTY: { price: 23800.00, change: 45.20, pct: 0.18, expiry: '2026-08-25' },
    SENSEX: { price: 77627.50, change: 861.66, pct: 1.12, expiry: '2026-08-06' }
  };

  // Smart date formatter (converts '2026-08-05' to '05 Aug')
  ...
  // Merge dashboard overrides with default metrics safely
  const marketData = {
  ...
  const activeExpiryStr = marketData.expiry;
  const [expiry, setExpiry] = useState(formatExpiryDate(activeExpiryStr));

  useEffect(() => {
    const rawExpiry = marketParameters && marketParameters[market] && marketParameters[market].expiry
      ? marketParameters[market].expiry
      : defaultParams[market].expiry;
    setExpiry(formatExpiryDate(rawExpiry));
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex items-center pt-3 pb-3 pl-3 border-b border-[#252b3d] bg-[#141824] shrink-0 z-20 shadow-sm">
                ...
                >
                  <div className="flex justify-between items-center mb-1.5">
                    ...
                    <span className="text-[9px] bg-[#252b3d] text-[#828b9d] px-1.5 py-0.5 rounded font-semibold tracking-wider">
                      Exp {formatExpiryDate(data.expiry)}
                    </span>
                  </div>
        ...
        {/* Expiry Dates Scroll */}
        <div className="flex space-x-2 overflow-x-auto hide-scrollbar px-4 py-3 bg-[#141824] border-b border-[#252b3d] shrink-0">
          {[formatExpiryDate(activeExpiryStr), '12 Aug', '19 Aug', '26 Aug', '30 Sep'].map((date, i) => {
            // Prevent duplicates if manual override date hits a default date
            if (i > 0 && date === formatExpiryDate(activeExpiryStr)) return null;

            return (
              <button
                key={i + date}
                onClick={() => setExpiry(date)}
                className={"px-4 py-1.5 rounded-full text-[11px] font-bold shrink-0 transition-colors " + (expiry === date ? 'bg-[#00D9B5] text-[#0B0E11] shadow-md shadow-[#00D9B5]/20' : 'border border-[#252b3d] text-[#828b9d] hover:text-white')}
              >
                {date}
              </button>
            )
        ...
        <div className="flex justify-between px-5 py-2.5 text-[10px] font-bold border-b border-[#252b3d] bg-[#181c2a] z-20 uppercase tracking-widest text-[#828b9d] shrink-0">
        ...
        <div className="flex-1 overflow-y-auto pb-8 hide-scrollbar bg-[#141824]">
          {strikes.map((row) => {
            // Exact Spot Line Placement Logic
            ...
            // CE Logic (Green if strike < spot, else Red)
            ...
            // PE Logic (Green if strike > spot, else Red)
            ...
            return (
              <React.Fragment key={row.strike}>
                ...
                {/* Draw Center Spot Line exactly where it belongs mathematically */}
                {showSpotLine && (
